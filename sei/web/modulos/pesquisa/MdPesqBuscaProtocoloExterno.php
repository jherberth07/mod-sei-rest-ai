 ...
 *
 * Classe de Busca na solu��o de indexa��o solr.
 *
 */
...
class MdPesqBuscaProtocoloExterno{
    ...
    public static function executar($parametrosSolr)
    {
      ...
      // FILTRA POR TIPO DE PROCESSO
        ...
        for ($i = 0; $i < count($registros); $i++) {
            ...
            // Protege contra a n�o idexa��o no solr quando o processo ou documento passa de p�blico para restrito ou quando o documento possui intima��es n�o cumpridas:
            if(!empty($objProtocoloDTO)){
            ...
            if ($objProtocoloDTO) {
                if ($objProtocoloDTO->getStrStaProtocolo() != ProtocoloRN::$TP_PROCEDIMENTO) {
                    ...
                    // Esconde highlight se intima��o do documento ou anexos n�o estiver cumprida:
                    ...
                    // INCLUIDO 21/12/2015 Substitui data de gera��o para data de assinatura de documentos gerados
            ...
            if ($objProtocoloDTO) {
                if(!$isPublico && !$bolLinkMetadadosProcessoRestrito) {
                    ...
                    if ($objHipoteseLegalDTO != null) {
                        $snippet = '<b>Hip�tese Legal de Restri��o de Acesso: ' . $objHipoteseLegalDTO->getStrNome() . ' (' . $objHipoteseLegalDTO->getStrBaseLegal() . ')</b>';
        ...
        //OS REGISTROS DA PAGINA PODE TER SIDO REMOVIDOS DEVIDO A UMA REGRA DA INTIMA��O NAO CUMPRIDA
    ...
    private static function retornoVazio($termo)
    {
        ...
        $semResultados .= "<div class=\"sem-resultado\">Sua pesquisa pelo termo <b>" .$termo. "</b> n�o encontrou nenhum protocolo correspondente. <br/><br/>Sugest�es:";
        $semResultados .= "<ul>";
    ...
    /**
     * Método adicional para API REST - pesquisa simplificada de processos
     */
    public function pesquisarProcessos($arrParams)
    {
        try {
            ...
            // Usa a pesquisa básica do ProtocoloRN para retornar resultados
